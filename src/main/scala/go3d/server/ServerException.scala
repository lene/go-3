...
class RateLimitExceeded(ip: String, maxRequests: Int, windowSecs: Long)
  extends ServerException(message = s"rate limit exceeded for $ip: max $maxRequests requests per ${windowSecs}s")
