...
class GoGridProtocol extends Thread {
	...
	if (gameStarted ()) {
		...
		int liberties = server.Liberty (x, y, z, player, false);
		...
		if (input.startsWith ("set at")) {
		    ...
		    boolean success = server.setStone (player+1, x, y, z);
	...
	//  requests which can be made only before game started
		...
		if (c < server.getNumPlayers ()) {
		    if (server.setColor (player, c)) {		//  set color, if not yet in use
		...
		if (h >= 2 && h <= GoGrid.MAX_HANDICAPS) {
		    ...
		    server.sendMessage (-1,		        //  inform all players about handicap for player
					"Player "+player+" has handicap "+server.getHandicap (player));
	...
	Utility.warning (e);
	server.sendMessage (player, e);                         //  send e to player
    }
